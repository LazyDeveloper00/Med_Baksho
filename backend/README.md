...
The included tests focus on the six pattern contracts and basic validation without requiring a live database:

```powershell
py -m unittest discover -s tests -v
```

For database regression testing, use a separate test database loaded from the corrected SQL file. Never run destructive tests against the shared team database.
