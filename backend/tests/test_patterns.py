...
class PatternRegressionTests(unittest.TestCase):
    ...
    def test_factory_role_dispatch_has_no_conditional_branch_nodes(self):
        # The current Factory is also a Singleton. __new__ legitimately needs one
        # initialization check, so the regression rule is scoped to role dispatch:
        # choosing patient/doctor/admin must stay registry-based with no branching.
        import inspect
        import textwrap
        from core.accounts.factories import AccountFactory

        source = inspect.getsource(AccountFactory.create_account)
    ...
    def test_account_factory_is_singleton(self):
        from core.accounts.factories import AccountFactory

        self.assertIs(AccountFactory.get_instance(), AccountFactory.get_instance())
